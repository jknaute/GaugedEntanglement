""" Numerical evaluation of second Renyi entropy in Z2 LGT using gauged PEPS transfer operator method
    ...
    In this file, the dependence on the deconfinement parameter gamma is analyzed.
"""
...
p2 = np.zeros(len(gamma_vals))
s2 = np.zeros(len(gamma_vals))
errors = []
...
# Write data to file:
output = np.concatenate((gamma_vals.reshape(len(gamma_vals),1), p2.reshape(len(p2),1), s2.reshape(len(s2),1)), axis=1)
np.savetxt("data/"+outputf, output, header="gamma   p2   s2; [Nx,Ny,R1,R2,a,b,d] = "+str([Nx,Ny,R1,R2,alpha,beta,delta]))
